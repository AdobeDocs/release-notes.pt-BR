...
|  | (Contas do [!DNL Google Ads]; versão de 11 de fevereiro) O suporte para a estratégia de oferta “[!UICONTROL Compartilhamento de impressões de público-alvo]” agora está disponível somente para campanhas na rede de pesquisa. Para essa estratégia de oferta, o [!DNL Google Ads] (não o [!DNL Search]) otimiza ofertas para alcançar um compartilhamento de impressões de público-alvo e uma posição de anúncio. Você também pode inserir um [!UICONTROL Compartilhamento de impressões de público-alvo] como uma porcentagem, a [!UICONTROL Posição de anúncio de público-alvo], e um [!UICONTROL CPC máximo] (custo por clique). Essa opção ainda não é permitida em portfólios híbridos. |
