---
title: Notas de versão anteriores - março de 2020
...
Notas de versão para a [!UICONTROL Experience Platform,] [!UICONTROL Experience Platform Launch,] [!UICONTROL Serviço de identidade,] Journey Orchestration, Mobile Services e informativos de segurança.
...
A versão do primeiro trimestre foi publicada. [Leia mais](https://experienceleague.adobe.com/docs/journeys/using/release-notes/release-notes.html?lang=pt-BR)
...
* [!DNL Target] - Agora a ID da [!DNL Target]sessão do Target será adicionada como parâmetro de dados de contexto `a.target.sessionId` na ocorrência interna do [!UICONTROL Analytics para Target] enviada para o Adobe Analytics.

**Android™ v4.18.1**

* [!DNL Target] - [!DNL Target] Agora a ID da sessão será adicionada como parâmetro de dados de contexto &quot;a.target.sessionId&quot; na ocorrência interna [!UICONTROL Analytics para Target] enviada para a Adobe Analytics.

## [!DNL Analytics] {#analytics}

Data de lançamento: **sexta-feira, 12 de março de 2020**
...
* Correção de um erro em que os clientes não podiam atualizar o nome do segmento devido à falta de permissão RBAC [!UICONTROL VIEW_ALL_DESTINATIONS]. A permissão [!UICONTROL VIEW_ALL_DESTINATIONS] não deve ser necessária para atualizar um segmento. Para obter mais informações sobre permissões RBAC, consulte [Administração (controles RBAC)](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/administration/administration-overview.html?lang=pt-BR). (AAM-52760)
* Correção de um erro no [Data Explorer](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/data-explorer/data-explorer-overview.html?lang=pt-BR), no qual alguns clientes não conseguiam visualizar o conteúdo na seção de informações básicas e os operadores no construtor de expressões, ao criar traços baseados nos sinais do [!UICONTROL Data Explorer]. (AAM-53130)
* Correção de um erro em que alguns clientes não podiam carregar a interface do [!UICONTROL Audience Marketplace]. (AAM-52070)
* Correção de um erro na [!UICONTROL API de segmentos], onde, devido a alguns segmentos sem descrição, a interface congelava quando os usuários tentavam acessar esses segmentos e os usuários tinham que sair dessa página. (AAM-53071)
...
O AEM 6.5, Service Pack 4.0 (6.5.4.0 lançado em 5 de março de 2020), é uma atualização importante que inclui novos recursos, principais melhorias para o cliente, melhor desempenho, estabilidade e segurança, lançados desde a disponibilização geral do AEM 6.5, em abril de 2019.
   * [Novidades do Adobe Experience Manager 6.5, Service Pack 4](https://experienceleague.adobe.com/docs/experience-manager-65/release-notes/service-pack/new-features-latest-service-pack.html?lang=pt-BR)
...
Consulte [Configurar o AEM Assets com o Brand Portal](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/publish/configure-aem-assets-with-brand-portal.html?lang=pt-BR).
...
| [!UICONTROL AEM Forms] | Artigo - [Autenticar para o autor do AEM usando OKTA](https://experienceleague.adobe.com/docs/experience-manager-learn/forms/single-sign-on-with-okta.html?lang=pt-BR) | Aprenda a configurar seu aplicativo no portal OKTA e saiba mais sobre as configurações que você geralmente usa para registrar o novo aplicativo. |
| [!UICONTROL AEM Commerce] | Tutorial - [Personalizar os componentes principais de CIF](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content-and-commerce/home.html?lang=pt-BR) | Analise vários pontos de extensão diferentes fornecidos pelos componentes principais de CIF e pelo AEM em geral. Os Componentes principais de CIF fornecem um conjunto padrão de componentes do Commerce que podem ser usados para acelerar um projeto que integra as soluções Adobe Experience Manager (AEM) e Magento. |
| [!DNL Adobe Campaign] - Destinos do público-alvo | Vídeo - [Criar um público-alvo...](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/profiles-and-audiences/audience-destinations/creating-audiences-using-segment-builder.html?lang=pt-BR) | Crie um público-alvo no Campaign Standard usando o [!UICONTROL Construtor de segmentos da Adobe Experience Platform]. Você pode acessar esse recurso diretamente no Adobe Campaign Standard por meio dos módulos do [!UICONTROL Audiences]. |
| [!DNL Adobe Campaign] - Destinos do público-alvo | Vídeo - [Ativação de públicos-alvo da Adobe Experience Platform em um fluxo de trabalho de marketing](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/profiles-and-audiences/audience-destinations/activating-aep-audiences.html?lang=pt-BR) | Aprenda a ativar o [!UICONTROL Público-alvo de consulta dos serviços de dados] em um fluxo de trabalho, usando a atividade [!UICONTROL Ler público-alvo]. |
| [!DNL Adobe Campaign] | Tutorial - [Notificação por push com Android™](https://experienceleague.adobe.com/docs/campaign-standard-learn/getting-started-with-push-notifications-android/introduction.html?lang=pt-BR) | Envie notificações por push personalizadas e segmentadas para dispositivos móveis iOS e Android™. Este tutorial o guiará pelas etapas que envolvem o envio de notificações por push do Adobe Campaign e no recebimento dessas notificações no aplicativo Android™. |
| [!DNL Adobe Campaign] | Vídeo - [Criar uma notificação por push](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/communication-channels/mobile/push/creating-a-push-notification.html?lang=pt-BR) | Crie uma notificação por push no Adobe Campaign Standard. Você pode enviar notificações por push personalizadas e segmentadas para dispositivos móveis iOS e Android™. |
| [!DNL Adobe Campaign] - Conector de dados AEP | Vídeo - [Verifique o status de um processo de ingestão de dados](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/adobe-experience-platform-data-connector/checking-status-of-data-ingestion-jobs.html?lang=pt-BR) | Aprenda a verificar o status de um processo de ingestão de dados e se os dados foram ingeridos do Adobe Campaign Standard para a Adobe Experience Platform. |
| [!DNL Adobe Campaign] - Conector de dados AEP | Vídeo - [Modificar o mapeamento de dados](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/adobe-experience-platform-data-connector/modifying-data-mapping.html?lang=pt-BR) | Aprenda a verificar o status e modificar o mapeamento de dados. |
| [!DNL Adobe Campaign] - Conector de dados AEP | Vídeo - [Mapear os eventos de experiência](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/adobe-experience-platform-data-connector/mapping-experience-events.html?lang=pt-BR) | Aprenda a mapear os eventos de experiência na Adobe Experience Platform. |
| [!DNL Adobe Campaign] - Conector de dados AEP | Vídeo - [Mapear os recursos personalizados](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/adobe-experience-platform-data-connector/mapping-custom-resources.html?lang=pt-BR) | Aprenda a mapear diferentes tipos de dados entre o Adobe Campaign Standard e a Adobe Experience Platform. |
| [!DNL Adobe Campaign] - Conector de dados AEP | Vídeo - [Noções básicas sobre o conector de dados da Adobe Experience Platform](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/adobe-experience-platform-data-connector/understanding-the-adobe-experience-platform-data-connector.html?lang=pt-BR) | Aprenda a disponibilizar os dados na Adobe Experience Platform, mapeando os dados XTK (dados assimilados no Campaign) para dados do Experience Data Model (XDM) na Adobe Experience Platform. |
| [!DNL Adobe Campaign] - Conector de dados AEP | Vídeo - [Mapear os dados da tabela de propagação](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/administrating/adobe-experience-platform-data-connector/mapping-seed-table-data.html?lang=pt-BR) | Aprenda a mapear os dados de propagação / perfis de teste com a Adobe Experience Platform. |
| [!DNL Adobe Campaign] - Destinos do público-alvo | Vídeo - [Alterar a dimensão de direcionamento de uma entrega para um Público-alvo da plataforma](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/profiles-and-audiences/audience-destinations/changing-targeting-dimension.html?lang=pt-BR) | Aprenda a alterar a dimensão de direcionamento de uma entrega para um Público-alvo da plataforma, fora da tabela de perfil principal no Adobe Campaign Standard. |
| [!DNL Adobe Campaign] | Vídeo - [Gerenciamento de big data no Snowflake](https://experienceleague.adobe.com/docs/campaign-classic-learn/tutorials/administrating/fda/big-data-segmentation-on-snowflake.html?lang=pt-BR) | Utilize o conector Snowflake no Adobe Campaign Classic. |
| [!DNL Adobe Campaign] - Destinos do público-alvo | Artigo - [Destinos do público-alvo (BETA) - Visão geral](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/profiles-and-audiences/audience-destinations/audience-destinations-overview.html?lang=pt-BR) | Aprenda a utilizar os dados de perfil centralizados e consolidados da Adobe Experience Platform para campanhas de marketing no Adobe Campaign Standard. |
| Adobe Analytics | Vídeo - [Adobe Summit 2019 Super Session](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/adobe-summit-2019-super-session-high-tech.html?lang=pt-BR) | Assista a clips preparados da &quot;super session&quot; de alta tecnologia na Summit 2019. |
| Adobe Analytics | Vídeo - [Adobe Summit 2019 Super Session](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/adobe-summit-2019-super-session-travel.html?lang=pt-BR) | Assista a clips preparados da sessão de viagem e hospitalidade do Summit 2019. |
| Adobe Analytics | Vídeo - [Adobe Summit 2019 Super Session](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/adobe-summit-2019-super-session-retail.html?lang=pt-BR) | Veja os clipes preparados da sessão de varejo no Summit 2019. |
| Adobe Analytics | Vídeo - [Caso de uso do cliente: Accent Group investe na experiência do cliente para impulsionar vendas](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/accent-group-invests-in-customer-experience-to-drive-sales.html?lang=pt-BR) | Veja como o Accent Group usa a Adobe Experience Cloud para criar experiências digitais perfeitas. |
| Adobe Analytics | Vídeo - [Caso de uso do cliente: ServiceNow obtém os insights certos para se conectar com clientes potenciais](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/servicenow-gets-the-right-insights-to-connect-with-prospects.html?lang=pt-BR) | Descubra como a [!DNL ServiceNow] obtém dados acionáveis de seus canais de marketing e aumenta o ROI em publicidade de pesquisa paga com a Adobe Advertising Cloud e o Adobe Analytics. |
| Adobe Analytics | Vídeo - [Adobe Analytics - É mais do que dados, é inteligência do cliente](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/adobe-analytics-customer-intelligence.html?lang=pt-BR) | Saiba mais sobre o marketing orientado por dados e como levar a maturidade da sua análise, dos dados aos insights para a ação. |
| Adobe Analytics | Vídeo - [Adobe AI e Adobe Analytics - Versão estendida](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/adobe-sensei-and-adobe-analytics.html?lang=pt-BR) | Veja os principais recursos do Adobe Analytics alimentados pela Adobe [!DNL AI,] incluindo [!UICONTROL Detecção de anomalias], [!UICONTROL Análise de contribuição], [!UICONTROL Alertas inteligentes], [!UICONTROL Clustering], [!UICONTROL IQ de segmentos], e [!UICONTROL Modelagem de propensão]. |
| Adobe Analytics | Vídeo - [Como a Adobe Analysis Workspace pode mudar seu negócio](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/how-adobe-analysis-workspace-can-change-your-business.html?lang=pt-BR) | Saiba como você pode realizar Ad Hoc Analysis, análise flexível, análise de coorte e análise de fallout usando o [!UICONTROL Analysis Workspace]. Você também pode compartilhar o ambiente de trabalho da análise com todos na sua empresa. Além disso, a função de arrastar e soltar permite que todos analisem os dados com facilidade e obtenham insights rapidamente. |
| Adobe Analytics | Vídeo - [Caso de uso do cliente: a Home Depot inova com o Gerenciamento de experiência do cliente](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/intro-to-analytics/what-can-aa-do-for-me/the-home-depot-innovates-with-customer-experience-management.html?lang=pt-BR) | Saiba como a [!DNL Home Depot] usa as soluções da Adobe para criar fidelidade à marca e satisfação do cliente com uma experiência personalizada de compras. |
| Adobe Analytics | Artigo - [Dicas do cliente para continuar sua jornada de aprendizado no Adobe Analytics](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/analysis-workspace/tips-and-tricks/customer-tips-for-continuing-your-adobe-analytics-learning-journey.html?lang=pt-BR) | Conheça três clientes da Adobe que têm dicas e truques para oferecer a você sobre como aproveitar ao máximo o Adobe Analytics. |
